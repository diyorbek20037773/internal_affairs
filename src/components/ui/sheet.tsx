...
>(({ className, children, side = "left", ...props }, ref) => (
  <SheetPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed z-50 h-full w-72 bg-sidebar text-sidebar-foreground p-0 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=open]:duration-300 data-[state=closed]:duration-200",
        side === "left"
          ? "left-0 top-0 data-[state=open]:slide-in-from-left data-[state=closed]:slide-out-to-left"
          : "right-0 top-0 data-[state=open]:slide-in-from-right",
        className
      )}
      {...props}
    >
